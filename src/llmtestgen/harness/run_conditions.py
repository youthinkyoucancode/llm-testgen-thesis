...
def run_condition_a(
    module: str | Path, human_tests: str | Path, *, timestamp: str | None = None
) -> ExperimentRecord:
    """Measure the existing human-written suite (the reference ceiling)."""
    code = Path(human_tests).read_text(encoding="utf-8")
    cov = measure_coverage(code, module)
    result = PipelineResult(final_tests=code, final_coverage=cov, iterations=[],
                            total_tokens=0, stop_reason="human reference suite")
    return build_record(result, module=Path(module).stem, model=_HUMAN, condition="A", timestamp=timestamp)


def run_condition_b(
    module: str | Path, *, model: ModelConfig, prompts_dir: str | Path,
    ...
    result = run_pipeline(module, model=model, config=single, prompts_dir=prompts_dir, generate_fn=generate_fn)
    return build_record(result, module=Path(module).stem, model=model, condition="B", timestamp=timestamp)


def run_condition_c(
    module: str | Path, *, model: ModelConfig, config: LoopConfig, prompts_dir: str | Path,
    generate_fn=generate, timestamp: str | None = None,
) -> ExperimentRecord:
    """The full iterative pipeline (the treatment)."""
    result = run_pipeline(module, model=model, config=config, prompts_dir=prompts_dir, generate_fn=generate_fn)
    return build_record(result, module=Path(module).stem, model=model, condition="C", timestamp=timestamp)
...
def run_module(
    ...
    if human_tests is not None:
        records.append(run_condition_a(module, human_tests, timestamp=timestamp))
    records.append(run_condition_b(
        module, model=model, prompts_dir=prompts_dir,
    ...
    records.append(run_condition_c(
        module, model=model, config=config, prompts_dir=prompts_dir,
        generate_fn=generate_fn, timestamp=timestamp,
    ))
    ...
    if results_dir is not None:
        ...
        for rec in records:
            write_json(rec, results_dir / f"{stem}_{rec.condition}.json")
